...
# Set paths
root_dir = r'D:\Duke_DLDS\Series_Classification\Series_Classification'
csv_path = r'D:\Duke_DLDS\SeriesClassificationKey.csv'
output_folder = r'D:\Duke_DLDS\Ensemble_output_SUNDAY'
...
class ImprovedFusionModel(nn.Module):
    def __init__(self, models, num_classes):
        ...
        # Cross-attention mechanism for model interaction
        embed_dim = 256
        self.model_embeddings = nn.ModuleList([
            nn.Sequential(
        ...
        # Dynamic weighting of models
        self.weights = nn.Parameter(torch.ones(len(models)))
        
        # Feature fusion layers with residual connections and dropouts
    ...
    def forward(self, x):
        ...
        # Apply dynamic weighting using softmax
        normalized_weights = torch.nn.functional.softmax(self.weights, dim=0)
        
        # Transform each model's output through embeddings
        embedded_features = []
        for i, output in enumerate(model_outputs):
            embedded = self.model_embeddings[i](output)
            weighted_embed = embedded * normalized_weights[i]
            embedded_features.append(weighted_embed)
        
        # Stack for self-attention
        stacked_features = torch.stack(embedded_features, dim=0)
        
        # Apply self-attention for cross-model interaction
        attn_output, _ = self.self_attention(
            stacked_features, stacked_features, stacked_features
        )
        
        # Combine attended features
        combined_features = torch.mean(attn_output, dim=0)
...
def train_model(model, model_name, train_loader, val_loader, criterion, optimizer, num_epochs, device='cuda'):
    model.to(device)
    best_val_acc = 0.0
...
def train_fusion(fusion_model, train_loader, val_loader, criterion, optimizer, num_epochs=5, device='cuda'):
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer,
        T_0=5,  # Initial restart interval (epochs)
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in tqdm(train_loader, desc=f"Fusion - Epoch {epoch + 1}/{num_epochs} - Training"):
            # Convert string labels to indices
            label_indices = torch.tensor([label_to_idx[label] for label in labels], dtype=torch.long)
        ...
        with torch.no_grad():
            for inputs, labels in tqdm(val_loader, desc=f"Fusion - Epoch {epoch + 1}/{num_epochs} - Validation"):
                # Convert string labels to indices
                label_indices = torch.tensor([label_to_idx[label] for label in labels], dtype=torch.long)
        ...
        if val_epoch_acc > best_val_acc:
            ...
            print(f"Saved new best fusion model with validation accuracy: {val_epoch_acc:.4f}")
    ...
    plt.plot(history['train_loss'], label='Train Loss')
    plt.plot(history['val_loss'], label='Validation Loss')
    plt.title('Fusion - Loss vs. Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    ...
    plt.plot(history['train_acc'], label='Train Accuracy')
    plt.plot(history['val_acc'], label='Validation Accuracy')
    plt.title('Fusion - Accuracy vs. Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()
    ...
    plt.subplot(1, 3, 3)
    plt.plot(history['learning_rates'], label='Learning Rate')
    plt.title('Fusion - Learning Rate vs. Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Learning Rate')
    plt.yscale('log')
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'fusion_training_history.png'))
...
def main():
    ...
    print("\n=== Creating Advanced Fusion Model ===")
    ...
    fusion_model = ImprovedFusionModel(fusion_models, len(label_to_idx))
    ...
    report, cm, fusion_metrics = evaluate_model(fusion_model, "fusion", test_loader, device=device)
